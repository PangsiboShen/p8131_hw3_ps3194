...
```
For the intercept, we first apply anti log to get valid number for interpretations. The odds for O. aegyptiaca 75 grown on Bean root extract media is -0.43; the odds ratio for growing in Bean root extract media between O. aegyptiaca 75 and O. aegyptiaca 73 is -0.2705; the odds ratio for O. aegyptiaca 75 growing in between Bean root extract media and Cucumber root extract media is 1.0647. Types of seed seems to have weak effect on the germination rate for the seeds since its p-value is 0.080435 which is greater than 0.05 yet less than 0.1.      
...
After I updated my model with dispersion parameter, all the beta coefficients stay the same.
